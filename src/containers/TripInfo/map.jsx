...
function GoogleMaps(props) {
  ...
  const updateStation = stationId => {
    dispatch({
      ...
      payload: {
        id: stationId,
        time: (new Date() - state.tripStartTime) / 1000
      }
    });
  };
  ...
  const moveBus = () => {
    ...
    if (!distance || state.tripStatus === "PENDING") {
      return;
    }
  ...
  useEffect(() => {
    if (state.tripStatus === "PENDING" && state.route.length > 0) {
      ...
      dispatch({ type: "UPDATE_DISTANCE", payload: newPath });
        ...
        {state.route.map((e, i) => {
          return (
            <Marker
              ...
              key={i}
              icon={{
                url: "https://c7.uihere.com/files/338/123/824/google-maps-google-map-maker-pin-clip-art-blue-map-pin-png.jpg",
...
export default props => {
  return (
    <MapComponent
      {...props}
      googleMapURL="https://maps.googleapis.com/maps/api/js?libraries=geometry,drawing,places&key=AIzaSyDec_AVHmasoVFZEwSngaIRE46ZbxjIcrs"
